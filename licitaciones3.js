...
(async () => {
  ...
  while (!isBtnDisabled) {
    ...
    const data = await page.evaluate(() => {
      ...
      items.forEach((item) => {
        results.push({
          ...
          objetivo: item.querySelector('td.tdTipoContratoLicOC').innerText,
          estado: item.querySelector('td.tdEstado').innerText,
          importe: item.querySelector('td.tdImporte').innerText,
          fecha: item.querySelector('td.tdFecha').innerText
        })
      })
      return results
    })
    ...
    // fs.appendFile('results.json', JSON.stringify(data), 'utf8', () =>
